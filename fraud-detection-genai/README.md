...
1. **Synthetic Data Generation** — Uses Generative AI to create realistic financial transaction datasets, including both legitimate and fraudulent patterns
2. **Framework Design** — Builds a structured pipeline from data ingestion to model evaluation
3. **Model Training** — Trains a machine learning model to identify fraudulent transaction patterns
...
├── data/
│   └── synthetic/             ← Generated transaction datasets
├── notebooks/
│   └── fraud_detection.ipynb  ← Main project notebook
├── src/
│   └── model.py               ← Model training and detection logic
└── results/
    └── evaluation_metrics.png ← Model performance visuals
```
